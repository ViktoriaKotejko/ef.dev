<?php
 /*
 Цикл одиночной записи без миниатюры
 */
 ?>
 
<?php $category = get_the_category( $post->ID);?> 
<?php echo is_wp_error( $cat_parents = get_category_parents($category[0]->cat_ID, TRUE, '<span> >> </span>') ) ? '' : '<div class="single_bread">'.$cat_parents.'</div>';?>		
<div class="clear"></div> 
<?php get_sidebar(); ?>
<div id="main-content" class="narrow">
	...
	<?php $old_price = get_post_meta($post->ID, 'old_price', true);?>
	<?php $similar_tag_name = get_post_meta($post->ID, 'similar_tag_name', true);?>
		
	<div id="single_wrapper">
		<?php $similar_products = get_post_meta($post->ID, 'similar_products', true); ?>
